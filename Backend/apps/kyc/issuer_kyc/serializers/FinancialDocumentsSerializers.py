...
#         company = CompanyInformation.objects.filter(pk=company_id).first()


#         doc_type = data['document_type']
#         doc_tag = data['document_tag']
...
#             raise serializers.ValidationError('Monthly/quarter values not allowed for yearly documents')

#         # --------------- BUSINESS RULES (INDIA) -------------------
...
#             if company.turnover > 50000000:
#                 pass
#             else:
#                 if period_type != PeriodType.YEARLY:
#                     raise serializers.ValidationError({'period_type': 'ITR must be yearly unless turnover > 5Cr'})
...
#             raise serializers.ValidationError({'file': 'Only PDF files allowed'})
...
#         # ✅ 1. company_id from context
#         company_id = self.context.get("company_id")
#         if not company_id:
#             raise serializers.ValidationError("company_id missing from context")

#         # ✅ 2. Fetch company object for turnover rules
...
#         # ✅ GSTR-3B must ALWAYS be monthly
#         if doc_type == DocumentType.GSTR_3B and period_type != PeriodType.MONTHLY:
#             raise serializers.ValidationError({'period_type': 'GSTR-3B must be monthly'})

#         # ✅ GSTR-9 must ALWAYS be yearly
#         if doc_type == DocumentType.GSTR_9 and period_type != PeriodType.YEARLY:
#             raise serializers.ValidationError({'period_type': 'GSTR-9 must be yearly'})

#         # ✅ ITR - monthly allowed only if turnover > 5 crore
#         if doc_type == DocumentType.ITR:
#             if company.turnover <= 50000000:  # 5 crore
#                 if period_type != PeriodType.YEARLY:
#                     raise serializers.ValidationError({
#                         'period_type': 'ITR must be yearly unless turnover > 5 Cr'
#                     })

#         # ✅ Financial Statements must ALWAYS be yearly
#         if doc_type == DocumentType.FINANCIAL_STATEMENT and period_type != PeriodType.YEARLY:
#             raise serializers.ValidationError({'period_type': 'Financial statements must be yearly'})
...
#         is_audited = (doc_tag == DocumentTag.AUDITED)
...
#                     raise serializers.ValidationError({
#                         'auditor_name': 'Required for audited documents'
#                     })

#                 if "audit_report_date" in data and not data.get("audit_report_date"):
#                     raise serializers.ValidationError({
#                         'audit_report_date': 'Required for audited documents'
#                     })

#             else:
#                 # CREATE: require both fields always
#                 if not data.get("auditor_name"):
#                     raise serializers.ValidationError({
#                         'auditor_name': 'Required for audited documents'
#                     })
