...
#     def extract_pan_number(self, file_bytes):
#         """Extract PAN number using OCR."""
#         try:
#             img = Image.open(BytesIO(file_bytes))
#             text = pytesseract.image_to_string(img).upper()
#             pan_match = re.search(r"[A-Z]{5}[0-9]{4}[A-Z]{1}", text)
#             return pan_match.group(0) if pan_match else None
#         except Exception as e:
#             print("PAN OCR error:", e)
#             return None
    

#     def extract_aadhaar_number(self, file_bytes):
#         try:
#             img = Image.open(BytesIO(file_bytes))
#             text = pytesseract.image_to_string(img)
#             text = text.replace("\n", " ").replace("\t", " ")
#             text = re.sub(r"[^\d\s]", "", text)  # keep only digits and spaces
...
#     def create(self, validated_data):
#         """Create a new signatory for the given company."""
#         request = self.context.get("request")
#         company_id = self.context.get("company_id")
...
#         extracted_pan = self.extract_pan_number(pan_bytes)
#         extracted_aadhaar = self.extract_aadhaar_number(aadhaar_bytes)

#         if not extracted_pan:
#             raise serializers.ValidationError(
#                 {"document_file_pan": "Unable to extract PAN number. Please upload a clearer image."}
#             )

#         if not extracted_aadhaar:
#             raise serializers.ValidationError(
#                 {"document_file_aadhaar": "Unable to extract Aadhaar number. Please upload a clearer image."}
#             )

#         # ✅ Check for duplicate PAN or Aadhaar across all signatories
#         if CompanySignatory.objects.filter(pan_number=extracted_pan).exists():
#             raise serializers.ValidationError(
#                 {"document_file_pan": f"This PAN number ({extracted_pan}) is already registered with another signatory."}
#             )
...
#         # Save files properly with unique filenames
#         timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
#         pan_filename = f"pan_{timestamp}.pdf"
#         aadhaar_filename = f"aadhaar_{timestamp}.pdf"
#         dsc_filename = f"dsc_{timestamp}.pdf" if dsc_file else None
...
#         # ✅ Create record only if validation passed
#         signatory = CompanySignatory.objects.create(
#             company=company,
#             name_of_signatory=validated_data["name_of_signatory"],
#             designation=validated_data["designation"],
...
#             "aadhaar_number": signatory.aadhaar_number,
#             "email_address": signatory.email_address,
#             "message": "Signatory added successfully with extracted details.",
#         }
