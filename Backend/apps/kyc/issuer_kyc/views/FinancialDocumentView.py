...
class FinancialDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # 🔥 UPDATED (replaces company_id from URL)
        company = get_company_from_token(self.request)

        if not company:
            return FinancialDocument.objects.none()

        qs = qs.filter(company_id=company.company_id, del_flag=0)
    ...
    # --------------------------------------------
    # No business logic below has been changed
    # Only company_id replaced with company.company_id
    # --------------------------------------------
    ...
    def create(self, request, *args, **kwargs):
        ...
        company_id = company.company_id   # keep SAME behaviour inside code
    ...
    def bulk_upload(self, request):
        ...
        # (business logic unchanged below…)
        # -----------------------------------------------------------
        # NO LINES CHANGED
        # -----------------------------------------------------------
        results = {"success": [], "failed": []}
