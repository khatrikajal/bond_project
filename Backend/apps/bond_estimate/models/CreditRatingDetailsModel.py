...
class CreditRatingDetails(BaseModel):
    """
    Stores credit rating details for a company.
    A company may have multiple credit ratings.
    """
    ...
    upload_letter = models.FileField(
        ...
        null=True,
        blank=True,
        help_text="Uploaded PDF of the official credit rating letter."
    )

    # Active / Inactive based on valid_till
    status = models.BooleanField(
        default=True,
        help_text="True = Active, False = Inactive. Auto-updated based on valid_till."
    )

    #  NEW FIELD: Verification status
    is_verified = models.BooleanField(
        default=False,
        help_text="True if the rating certificate is verified via API or by an authorized person."
    )
    ...
    remark = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Optional remarks about verification (manual or API response)."
    )
    ...
    # Auto-update status based on valid_till
    def save(self, *args, **kwargs):
        self.status = self.valid_till >= date.today()
