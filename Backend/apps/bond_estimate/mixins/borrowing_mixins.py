...
class BulkOperationMixin:
    ...
    def bulk_delete(self, request, *args, **kwargs):
        ...
        ids = request.data.get('ids', [])
        if not ids:
            return APIResponse.error(message="No IDs provided")
        ...
        if updated_count == 0:
            return APIResponse.error(
                message="No matching borrowings found for given IDs",
                errors={"ids": ids},
        ...
        return APIResponse.success(
            ...
            data={"deleted_count": updated_count}
        )
    ...
    def bulk_restore(self, request, *args, **kwargs):
        ...
        if updated_count == 0:
            return APIResponse.error(
                message="No deleted borrowings found for these IDs",
                errors={"ids": ids},
                status_code=status.HTTP_404_NOT_FOUND
            )

        return APIResponse.success(
            message=f"Successfully restored {updated_count} records",
            data={"restored_count": updated_count}
        )
